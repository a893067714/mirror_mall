...
public class AddCartServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<CartQueryView> CartQueryViews = cartService.list(userInfo.getUserId());
		BigDecimal totalPrice = new BigDecimal(0);
		for (CartQueryView cartQueryView : CartQueryViews) {
		...
		request.setAttribute("CartQueryViews", CartQueryViews);
